...
class Mamoa:
    def __init__(self, pX, pY, prob, bb, seg):
        self.pX = pX
        self.pY = pY
        self.prob = prob
        self.bb = bb
            ...
            if res:
                for mamoa in res:
                    ...
                    for point in mamoa:
                        point[0] += xmin
                        point[1] += ymin
                    bb_aux.append(min([m[i] for m in mamoa] for i in range(1)))
                    bb_aux.append(min([m[i] for m in mamoa] for i in range(2)))
                    bb_aux.append(max([m[i] for m in mamoa] for i in range(1)))
                    bb_aux.append(max([m[i] for m in mamoa] for i in range(2)))
                    cv2.rectangle(im, (int(bb_aux[0]), int(bb_aux[1])), (int(bb_aux[2]), int(bb_aux[3])), (0, 0, 255), 1)
